...
class Graph:
    ...
    def compute_depth(self):
        ...
        def dfs_depth(vertex_index):
            visited = self.vertices[vertex_index]

            if visited.depth != -1:
                return visited.depth

            if not self.get_adjacent_vertices(vertex_index):
                visited.depth = 0
            else:
                max_d = 0
                for neighbor in self.get_adjacent_vertices(vertex_index):
                    d = dfs_depth(neighbor)
                    max_d = max(max_d, d)
                visited.depth = max_d + 1
            return visited.depth
    ...
    def has_cycle(self):
        ...
        number_vert = len(self.vertices)
        visited = [False] * number_vert
        current_path = [False] * number_vert

        def visit(u):
            visited[u] = True
            current_path[u] = True

            for vertex in self.get_adjacent_vertices(u):
                if not visited[vertex]:

                    if visit(vertex):
                        return True

                elif current_path[vertex]:
                    return True
        ...
        for vert_index in range(number_vert):
            if not visited[vert_index]:
                if visit(vert_index):
                    return True
    ...
    def get_registration_plan(self):
        ...
        in_degree = [0] * n
        for u in range(n):
            neighbors = self.get_adjacent_vertices(u)
            for v in neighbors:
                in_degree[v] += 1
        ...
        while remaining > 0:
            ...
            while len(semester) < 4 and len(available) > 0:
                best_course_index = 0
                for i in range(1, len(available)):
                    if self.vertices[available[i]].depth > \
                        self.vertices[available[best_course_index]].depth:
                        best_course_index = i

                course_index = available.pop(best_course_index)

                semester.append(self.vertices[course_index].label)
                processed[course_index] = True
                remaining -= 1

                neighbors = self.get_adjacent_vertices(course_index)
...
def main():
    ...
    num_strip = sys.stdin.readline().strip()
    num_vertices = int(num_strip)
    ...
    edges_strip = sys.stdin.readline().strip()
    num_edges = int(edges_strip)
    ...
    for _ in range(num_edges):
        ...
        start_label, end_label = line.split()

        x = graph.get_index(start_label)
        y = graph.get_index(end_label)
        graph.add_edge(x, y)
